...
func (m *Mob) Move(w WorldShared) {
	...
	if night && !m.HasTarget() {
		if pid, found := w.FindNearbyPlayer(m); found {
			m.SetTarget(pid)
		}
	}
...
func (m *Mob) tickKnockback(w WorldShared) {
	const drag = 0.91
	const gravity = 0.08
...
func (m *Mob) resolveGroundCollision(w WorldShared, newX, newY, newZ, vy float64) (float64, float64, bool) {
	if vy > 0 {
...
func (m *Mob) checkObstacles(w WorldShared, x, y, z float64, vx, vz float64) (blockedX, blockedZ bool) {
	const lookAhead = 0.3
...
func (m *Mob) resolveVerticalVelocity(w WorldShared, x, y, z float64, vx, vz float64, blockedAhead bool) float64 {
...
func (m *Mob) adjustForOthers(w WorldShared) (avoidX, avoidZ float64) {
